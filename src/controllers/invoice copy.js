...
Terimakasih
    ...
    GetInvoicev2: async (req, res) => {
        ...
        try {
            ...
            const invoice_query = { is_active: 1 };
            if (id) invoice_query._id = id;
            if (search) {
                Object.assign(invoice_query, {
                  $or: [
            ...
            // if (result) invoice_query.invoice_type = { $in: result };
            const invoiceQuery = Invoice.find(invoice_query);
            if (start_range && typeof start_range === "number") invoiceQuery.skip(start_range);
            if (end_range && typeof end_range === "number") invoiceQuery.limit(end_range);
            const invoices = await invoiceQuery.exec();
            const resultPromises = invoices.map(async (invoice) => {
                const obj_result = {};
                const trn_do_01 = await TrnDo01.findOne({ do_no: invoice.doc_ref }, { partner_id: 1 }).lean();
                console.log(trn_do_01)
                obj_result.invoice_no = invoice.invoice_no
                obj_result.partner_id = trn_do_01.partner_id
                return obj_result;
            });
    ...
    invoicePaymentBulk: async (req, res) => {
        try {
            const {
                ids,
                payment_type
            ...
            return res.json({
                status: "success",
                message: "berhasil payment invoice",
                data: []
            })

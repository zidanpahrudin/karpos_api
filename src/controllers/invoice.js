...
Terimakasih
    ...
    GetInvoicev2: async (req, res) => {
        ...
        try {
            ...
            if (invoice_status) invoice_pipeline.push({ $match: { invoice_status: { $in: invoice_status.split(',') } } });
            invoice_pipeline.push(
                {
                    $graphLookup: {
                        from: 'mst_warehouse',
            ...
            if (search) {
                ...
                if(invoice_status) {
                    total_count_invoice = await Invoice.countDocuments({is_active: 1, invoice_status: { $in: invoice_status.split(',') } })
                } else {

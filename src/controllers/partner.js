...
module.exports = {
    ...
    mobileGetProfile: async (req, res) => {
        try {
            ...
            const user = await UserV2.findOne({_id: req.user.id});
            const objectProfile = {};
            if(user) {
                const company = await Company.findOne(
                ...
                if(company) {
                    objectProfile.company_name = company.pt_name;
                    objectProfile.company_logo = company.tautan + "/" + company.logo;
                    for (const rekening of company.rekening) {
                        if(rekening.is_select === 1 && rekening.is_active === 1) {
                            objectProfile.rek_no = rekening.rek_no;
                            objectProfile.rek_name = rekening.rek_name;
                        }
                    }
        ...
        try {
            ...
            bcrypt.genSalt(10, function (err, salt) {
                ...
                bcrypt.hash(confirm_password, salt, async function (err, hash) {
                    ...
                    await UserV2.findByIdAndUpdate(req.user.id, {
                        password: hash
                    });
